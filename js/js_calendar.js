...
const calendarGrid = Ril document.getElementById('calendarGrid');
...
function renderCalendar() {
    ...
    for (let i = 0; i < monthData.days; i++) {
        ...
        dayDiv.innerHTML = `
            ...
            <span class="nepali-date">${i + 1}</span>
            <span class="gregorian-date">${new Date(year - 57, currentMonthIndex, i + 1).toISOString().split('T')[0]}</span>
        `;
        dayDiv.addEventListener('click', () => {
            document.getElementById('selectedDate').textContent = `${year}-${String(currentMonthIndex + 1).padStart(2, '0')}-${String(i + 1).padStart(2, '0')}`;
